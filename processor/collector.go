...
func getExifTimeField(metaData *exif.Exif, fieldName exif.FieldName, fallbackTime time.Time) *time.Time {
	...
	fieldDate, err := time.ParseInLocation("2006:01:02 15:04:05", fieldStr, time.Local)

	if err != nil {
		log.Printf("\terror converting value of %s to string: %s. Going to use defaultTime %s.", fieldName, err, fallbackTime)
